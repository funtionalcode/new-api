...
test('custom providers have their own tab and keep the search when switching categories', async () => {
  ...
  expect(
    within(panel).getByRole('option', { name: 'Advanced Custom Built-in #60' })
  ).toBeVisible()
  expect(
  ...
  expect(
    screen.queryByRole('option', { name: 'Advanced Custom Built-in #60' })
  ).not.toBeInTheDocument()
  expect(
    screen.queryByRole('option', { name: 'Custom Built-in #8' })
  ).not.toBeInTheDocument()
  await user.type(screen.getByRole('combobox'), 'Advanced Custom')
  expect(screen.queryByRole('option')).not.toBeInTheDocument()
  await user.click(within(tabs).getByRole('tab', { name: 'Custom' }))
  expect(screen.getByRole('combobox')).toHaveValue('Advanced Custom')
  expect(screen.getAllByRole('option')).toHaveLength(1)
  expect(
    screen.getByRole('option', { name: 'Advanced Custom Built-in #60' })
  ).toBeVisible()
})
...
test('deprecated and flexible integration badges preserve provider selection and expose the full explanation', async () => {
  ...
  const advanced = screen.getByRole('option', {
    name: 'Advanced Custom Built-in #60',
  })
  ...
  expect(select).toHaveBeenNthCalledWith(2, { kind: 'builtin', type: 60 })
