...
export function getDashboardChartColors(domainLength: number): string[] {
  const fallbackScheme = vchartDefaultDataScheme.at(-1)
  const scheme =
...
function formatUserRemarkLabel(username: string, remark?: string): string {
  const trimmedRemark = remark?.trim()
  if (!trimmedRemark) return username
  return `${username}\n${trimmedRemark}`
}
...
export function processChartData(
  ...
  return {
    ...
    // Token charts
    spec_token_pie: (() => {
      const tokenPieValues = [...modelTotalsMap.entries()]
        .map(([model, stats]) => ({
          type: model,
    ...
    spec_token_rank_bar: (() => {
      ...
      const allTokenRanks = [...modelTotalsMap.entries()]
        .map(([model, stats]) => ({
          Model: model,
          Tokens: Number(stats.tokens) || 0,
        }))
...
export function processUserChartData(
  ...
  data.forEach((item) => {
    ...
    userQuotaTotal.set(username, prev + getMetricValue(item))
    const remark = item.remark?.trim()
    if (remark && !userRemarkMap.has(username)) {
      userRemarkMap.set(username, remark)
    }
  })
  ...
  data.forEach((item) => {
    ...
    userMap.set(user, (userMap.get(user) || 0) + getMetricValue(item))
  })
      ...
      axes: [
        {
          ...
          label: {
            formatMethod: (value: string | number) => {
              ...
              return formatUserRemarkLabel(
                username,
                userRemarkMap.get(username)
              )
            },
            style: {
              fontSize: 11,
        ...
        },
        {
          ...
          type: 'linear',
          visible: false,
          title: { visible: false, text: metricTitle },
        },
      ],
      tooltip: {
        mark: {
      ...
      tooltip: {
        ...
        dimension: {
          ...
          updateContent: (
            ...
            for (let i = 0; i < array.length; i++) {
              const v = Number(array[i].value) || 0
              const remark = String(array[i].datum?.Remark || '').trim()
              sum += v
              if (remark) {
                array[i].key = `${array[i].key} - ${remark}`
              }
