...
type TokenChartSpecKey =
  | 'spec_token_line'
  | 'spec_token_pie'
  | 'spec_token_rank_bar'

const TOKEN_CHART_SPEC_KEYS: Record<TokenAnalyticsChartTab, TokenChartSpecKey> =
  {
    trend: 'spec_token_line',
...
interface TokenChartsProps {
  ...
  defaultChartTab?: TokenAnalyticsChartTab
}
...
export function TokenCharts(props: TokenChartsProps) {
  ...
  const [activeTab, setActiveTab] = useState<TokenAnalyticsChartTab>(
      ...
      <div className='flex w-full flex-col gap-1.5 border-b px-3 py-2 sm:gap-3 sm:px-5 sm:py-3 lg:flex-row lg:items-center lg:justify-between'>
        <div className='flex items-center gap-2'>
          ...
          <span className='text-muted-foreground text-xs'>
            {t('Total:')} {chartData.totalTokenDisplay}
          </span>
        </div>

        <div className='bg-muted/60 inline-flex h-7 w-full overflow-x-auto rounded-lg border p-0.5 sm:h-8 sm:w-auto'>
          {TOKEN_ANALYTICS_CHART_OPTIONS.map((tab) => (
            <button
              key={tab.value}
              type='button'
