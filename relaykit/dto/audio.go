...
func (r *AudioRequest) IsStream(req *http.Request) bool {
	...
	if req.MultipartForm != nil {
		for _, value := range req.MultipartForm.Value["stream"] {
			if stream, err := strconv.ParseBool(strings.TrimSpace(value)); err == nil && stream {
				return true
			}
		}
	}
	if req.PostForm != nil {
		for _, value := range req.PostForm["stream"] {
			if stream, err := strconv.ParseBool(strings.TrimSpace(value)); err == nil && stream {
				return true
			}
		}
	...
	if req.Form != nil {
		for _, value := range req.Form["stream"] {
			if stream, err := strconv.ParseBool(strings.TrimSpace(value)); err == nil && stream {
				return true
			}
		}
