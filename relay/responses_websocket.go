...
func (s *responsesWSSession) runCall(c *gin.Context, state *responsesWSCallState, create responsesWSCreateRequest) (apiErr *types.NewAPIError) {
	...
	service.PrepareCursorAgentSessionFromBody(c, modelName, create.Body)
	...
	if s.lockedChannelID != 0 {
		...
		if common.GetContextKeyInt(c, appconstant.ContextKeyChannelType) == appconstant.ChannelTypeCursor {
			return s.runCursorCall(c, state, create)
		}
		...
		for ; retry.GetRetry() <= common.RetryTimes; retry.IncreaseRetry() {
			...
			if channel.Type == appconstant.ChannelTypeCursor {
				s.lockChannel(c, channel, modelName, common.GetContextKeyString(c, appconstant.ContextKeyUsingGroup))
				return s.runCursorCall(c, state, create)
			}
			if info == nil {
				info = relaycommon.GenRelayInfoResponses(c, &create.Request)
			...
			s.lockChannel(c, channel, modelName, info.UsingGroup)
	...
	for {
		...
		case incoming := <-state.inbox:
			...
			if incoming.err != nil {
				...
				state.closeAfter = true
				code, reason := websocket.CloseInternalServerErr, "upstream websocket disconnected"
				var closed *websocket.CloseError
				if errors.As(incoming.err, &closed) && closed.Code != websocket.CloseAbnormalClosure && closed.Code != websocket.CloseNoStatusReceived {
					code, reason = closed.Code, closed.Text
				}
				state.terminal = &responsesWSMessage{kind: websocket.CloseMessage, body: websocket.FormatCloseMessage(code, reason)}
				ConsumeResponsesQuota(c, info, accumulator.Finish())
				return nil
			}
			info.SetFirstResponseTime()
			if info.TypeSafeObserve != nil {
				info.TypeSafeObserve(incoming.body)
			}
			var event struct {
...
func (s *responsesWSSession) lockChannel(c *gin.Context, channel *appmodel.Channel, modelName, group string) {
	s.lockedModel, s.lockedChannelID, s.lockedGroup = modelName, channel.Id, group
	s.lockedKey, s.lockedKeyIndex = common.GetContextKeyString(c, appconstant.ContextKeyChannelKey), common.GetContextKeyInt(c, appconstant.ContextKeyChannelMultiKeyIndex)
...
// Cursor 通过 HTTP 流提供 Responses 事件，并复用每条请求的鉴权和结算。
func (s *responsesWSSession) runCursorCall(c *gin.Context, state *responsesWSCallState, create responsesWSCreateRequest) (apiErr *types.NewAPIError) {
	create.Request.Stream = common.GetPointer(true)
	info := relaycommon.GenRelayInfoResponses(c, &create.Request)
	info.IsStream, info.IsWebsocket, info.DisablePing = true, true, true
	info.InitChannelMeta(c)
	write := func(data []byte) error {
		if info.TypeSafeObserve != nil {
			info.TypeSafeObserve(data)
		}
		if create.StreamID != "" {
			var err error
			data, err = sjson.SetBytes(data, "stream_id", create.StreamID)
			if err != nil {
				return err
			}
		}
		...
		case "response.completed", "response.done", "response.incomplete", "response.failed", "response.cancelled", "response.canceled":
			s.lastResponseID = gjson.GetBytes(data, "response.id").String()
			state.terminal = &responsesWSMessage{kind: websocket.TextMessage, body: bytes.Clone(data)}
			return nil
		}
		return s.writeClient(websocket.TextMessage, data)
	}
	if string(create.Generate) == "false" {
		if err := completeResponsesWebsocketPrewarm(&responsesWebsocketTurn{info: info}, write); err != nil {
			return types.NewError(err, types.ErrorCodeBadResponse, types.ErrOptionWithSkipRetry())
		}
		return nil
	}
	if _, err := helper.ValidateResponsesRequest(&create.Request, true); err != nil {
		return newResponsesWSInvalidRequestError(err)
	}
	defer func() {
		if recovered := recover(); recovered != nil {
			apiErr = types.NewError(fmt.Errorf("Cursor websocket request panic: %v", recovered), types.ErrorCodeBadResponse, types.ErrOptionWithSkipRetry())
			state.closeAfter = true
		}
		apiErr = RefundFailedRequestBilling(c, info, apiErr)
	}()
	...
	restore := helper.SetResponsesStreamWriter(c, write)
	defer restore()
...
func (s *responsesWSSession) restoreConnectionContext(c *gin.Context, model string) *types.NewAPIError {
	...
	if !channel.IsOpenToUser(c.GetInt("id")) {
		return types.NewErrorWithStatusCode(errors.New("channel is not open to the current user"), types.ErrorCodeAccessDenied, http.StatusForbidden, types.ErrOptionWithSkipRetry())
	}
...
func checkResponsesWSModelAccess(c *gin.Context, modelName string) *types.NewAPIError {
	if !middleware.IsModelAllowedByUser(c, modelName) {
		return types.NewErrorWithStatusCode(errors.New("user is not allowed to use this model"), types.ErrorCodeAccessDenied, http.StatusForbidden, types.ErrOptionWithSkipRetry())
	}
	if userID := c.GetInt("id"); userID > 0 {
		limit, err := appmodel.CheckUserTokenLimit(userID, time.Now())
		if err != nil {
		...
		if limit.Exceeded {
			return types.NewErrorWithStatusCode(errors.New(limit.Message()), types.ErrorCodeInsufficientUserQuota, http.StatusForbidden, types.ErrOptionWithSkipRetry())
		}
	}
	...
	if !middleware.TokenModelLimitAllows(tokenModelLimit, modelName) && !appmodel.IsModelAllowedByUserLimit(modelName, tokenModelLimit) {
...
func selectResponsesWSChannel(c *gin.Context, modelName string, retryParam *service.RetryParam) (*appmodel.Channel, *types.NewAPIError) {
	...
	constraints.AddFilter(appdto.ChannelFilter{Kind: appdto.FilterUserAccess, UserId: c.GetInt("id")})
	...
	if strings.TrimSpace(c.GetHeader(appconstant.CursorAgentIDHeader)) != "" {
		return middleware.SelectChannelForWebsocketRequest(c, modelName)
	}
