...
func formatLogOtherJSON(value string, visibility logOtherVisibility) string {
	...
	if visibility == logOtherVisibilityUser {
		if _, exists := values["typesafe_exchange"]; exists {
			delete(values, "typesafe_exchange")
			changed = true
		}
		var rawSummary any
		_ = common.Unmarshal(values["typesafe"], &rawSummary)
		var admin map[string]any
		if common.Unmarshal(values[logOtherAdminInfoKey], &admin) == nil && admin["typesafe"] != nil {
			rawSummary = admin["typesafe"]
		}
		if rawSummary != nil {
			if summary := SanitizeTypeSafeResults(rawSummary); len(summary) > 0 {
				if encoded, err := common.Marshal(summary); err == nil {
					values["typesafe"] = encoded
				}
			} else {
				delete(values, "typesafe")
			}
			changed = true
		}
