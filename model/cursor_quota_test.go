...
func TestUpdateCursorQuotaBindingUsagePreservesLastSuccessOnFailure(t *testing.T) {
	...
	binding := &CursorQuotaBinding{
		...
		LastGrokBotUsagePercent:   21.5,
		LastGrokBotResetAt:        1_786_838_400,
		LastGrokBotUsageAvailable: true,
	...
	updated, err := UpdateCursorQuotaBindingUsage(binding.Id, CursorQuotaUsageRefreshUpdate{
		...
		LastGrokBotUsagePercent: 1,
		LastGrokBotResetAt:      1,
	...
	assert.InDelta(t, 21.5, updated.LastGrokBotUsagePercent, 0.000001)
	assert.Equal(t, int64(1_786_838_400), updated.LastGrokBotResetAt)
	assert.True(t, updated.LastGrokBotUsageAvailable)
