...
func RecordLogWithAdminInfo(userId int, logType int, content string, adminInfo *AuditAdminInfo, operation *AuditOperation, request ...*gin.Context) {
	...
	if len(request) > 0 && request[0] != nil {
		if adminInfo == nil || adminInfo.AdminID == userId {
			log.Ip = common.GetClientIP(request[0])
		}
...
func GetAllLogs(logType int, startTimestamp int64, endTimestamp int64, modelName string, username string, tokenName string, startIdx int, num int, channel int, channelName string, group string, ip string, requestId string, upstreamRequestId string, visibleChannelIDs []int) (logs []*Log, total int64, err error) {
	...
	if tx, err = applyLogSearchFilter(tx, "logs.username", username); err != nil {
		return nil, 0, err
	}
	if tx, err = applyLogSearchFilter(tx, "logs.token_name", tokenName); err != nil {
		return nil, 0, err
	}
	if tx, err = applyLogSearchFilter(tx, "logs.ip", ip); err != nil {
		return nil, 0, err
	}
	if requestId != "" {
...
func SumUsedQuota(logType int, startTimestamp int64, endTimestamp int64, modelName string, username string, tokenName string, channel int, channelName string, group string, ip string, avgStartTimestamp int64, avgEndTimestamp int64, requestId string, upstreamRequestId string, visibleChannelIDs []int) (stat Stat, err error) {
	...
	if rpmTpmQuery, err = applyLogSearchFilter(rpmTpmQuery, "username", username); err != nil {
		return stat, err
	}
	if avgUseTimeQuery, err = applyLogSearchFilter(avgUseTimeQuery, "username", username); err != nil {
		return stat, err
	}
	if tx, err = applyLogSearchFilter(tx, "token_name", tokenName); err != nil {
		return stat, err
	}
	if rpmTpmQuery, err = applyLogSearchFilter(rpmTpmQuery, "token_name", tokenName); err != nil {
		return stat, err
	}
	if avgUseTimeQuery, err = applyLogSearchFilter(avgUseTimeQuery, "token_name", tokenName); err != nil {
		return stat, err
	}
	if tx, err = applyLogSearchFilter(tx, "ip", ip); err != nil {
		return stat, err
	}
	if rpmTpmQuery, err = applyLogSearchFilter(rpmTpmQuery, "ip", ip); err != nil {
		return stat, err
	}
	if avgUseTimeQuery, err = applyLogSearchFilter(avgUseTimeQuery, "ip", ip); err != nil {
		return stat, err
	}
	if requestId != "" {
		tx = tx.Where("request_id = ?", requestId)
		rpmTpmQuery = rpmTpmQuery.Where("request_id = ?", requestId)
		avgUseTimeQuery = avgUseTimeQuery.Where("request_id = ?", requestId)
	}
	if upstreamRequestId != "" {
		tx = tx.Where("upstream_request_id = ?", upstreamRequestId)
		rpmTpmQuery = rpmTpmQuery.Where("upstream_request_id = ?", upstreamRequestId)
		avgUseTimeQuery = avgUseTimeQuery.Where("upstream_request_id = ?", upstreamRequestId)
	}
	if startTimestamp != 0 {
	...
	if rpmTpmQuery, err = applyLogSearchFilter(rpmTpmQuery, "model_name", modelName); err != nil {
		return stat, err
	}
	...
	if group != "" {
		...
		avgUseTimeQuery = avgUseTimeQuery.Where(logGroupCol+" = ?", group)
	}

	if logType != LogTypeUnknown {
		tx = tx.Where("type = ?", logType)
		rpmTpmQuery = rpmTpmQuery.Where("type = ?", logType)
		avgUseTimeQuery = avgUseTimeQuery.Where("type = ?", logType)
	}
	avgUseTimeQuery = avgUseTimeQuery.Where("use_time > ?", 0)
	...
	if avgEndTimestamp == 0 {
		avgEndTimestamp = endTimestamp
	}
	if avgStartTimestamp != 0 {
		avgUseTimeQuery = avgUseTimeQuery.Where("created_at >= ?", avgStartTimestamp)
	}
	if avgEndTimestamp != 0 {
		avgUseTimeQuery = avgUseTimeQuery.Where("created_at <= ?", avgEndTimestamp)
	}
