...
func TestListSystemTasksFilterByType(t *testing.T) {
	...
	all, _, err := ListSystemTasks(SystemTaskFilter{}, 0, 20)
	require.NoError(t, err)
	require.GreaterOrEqual(t, len(all), 2)

	filtered, _, err := ListSystemTasks(SystemTaskFilter{Type: SystemTaskTypeDBBackup}, 0, 20)
	require.NoError(t, err)
	require.Len(t, filtered, 1)
	assert.Equal(t, backup.TaskID, filtered[0].TaskID)

	filteredCleanup, _, err := ListSystemTasks(SystemTaskFilter{Type: SystemTaskTypeLogCleanup}, 0, 20)
	require.NoError(t, err)
