...
func recordManageAuditFor(c *gin.Context, targetUserId int, action string, params map[string]any) {
	...
	model.RecordOperationAuditLog(operatorUserId, c.GetInt("role"), auditContentEN(action, params), common.GetClientIP(c), action, params, auditOperatorInfo(c), nil, c)
	markAuditLogged(c)
}
...
func recordUserSecurityAudit(c *gin.Context, userId int, action string, params map[string]any) {
	...
	model.RecordOperationAuditLog(userId, c.GetInt("role"), auditContentEN(action, params), common.GetClientIP(c), action, params, nil, auditInfo, c)
}
