...
`, key, baseURLField)
	...
	body := func(pluginKey string) string {
		return fmt.Sprintf(`{"mode":"single","channel":{"type":64,"name":"%s","key":"sk","models":"doc","group":"default","setting":"{\"task_plugin_key\":\"%s\"}"}}`, pluginKey, pluginKey)
	}
...
`
	...
	bound := `{"mode":"single","channel":{"type":62,"name":"gateway","key":"sk","models":"gateway-doc","group":"default","base_url":"https://gateway.example","setting":"{\"task_extend_plugin_keys\":[\"gateway-bind\"]}"}}`
	unbound := `{"mode":"single","channel":{"type":62,"name":"plain-gateway","key":"sk","models":"gpt","group":"default","base_url":"https://gateway.example"}}`
	...
	update := func(name, setting string) string {
		return fmt.Sprintf(`{"id":%d,"type":62,"name":%q,"key":"sk","models":"gateway-doc","group":"default","base_url":"https://gateway.example","setting":%q}`, channel.Id, name, setting)
	}
