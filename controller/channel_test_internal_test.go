...
func TestChannelTestPreservesCompactModelVariant(t *testing.T) {
	modelName := ratio_setting.WithCompactModelSuffix("gpt-5.6-sol")
	channel := &model.Channel{Type: constant.ChannelTypeOpenAI}

	assert.Equal(
		t,
		string(constant.EndpointTypeOpenAIResponseCompact),
		normalizeChannelTestEndpoint(channel, modelName, ""),
	)
	request, ok := buildTestRequest(modelName, "", channel, false).(*dto.OpenAIResponsesCompactionRequest)
	require.True(t, ok)
