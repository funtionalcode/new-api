...
func TestResponsesWebSocketDialsNativeResponsesChannelTypes(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "xai", channelType: constant.ChannelTypeXai, want: bearer},
...
func TestResponsesWebSocketCursorPrewarmAndHTTPFallback(t *testing.T) {
	fixture := newResponsesWSBillingTest(t, `tier("request", fixed(0.002))`, func(*websocket.Conn, *http.Request) {
		t.Error("Cursor must use its HTTP adaptor")
	})
	...
	fixture.httpUpstream = func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodGet && strings.HasSuffix(r.URL.Path, "/usage"):
			_, _ = w.Write([]byte(`{"runs":[{"id":"run-1","usage":{"inputTokens":10,"outputTokens":5,"totalTokens":15}}]}`))
	...
	require.NoError(t, model.DB.Model(fixture.channel).Updates(map[string]any{"type": constant.ChannelTypeCursor, "model_mapping": `{"ws-billing":"composer-2"}`}).Error)
	require.NoError(t, fixture.client.WriteMessage(websocket.TextMessage, []byte(`{"type":"response.create","model":"ws-billing","generate":false,"stream_id":"prewarm"}`)))
	assert.Equal(t, "response.created", readResponsesWSTestEvent(t, fixture.client)["type"])
	terminal := readResponsesWSTestEvent(t, fixture.client)
	require.Equal(t, "response.completed", terminal["type"])
	assert.Equal(t, "prewarm", terminal["stream_id"])
	assert.Zero(t, calls.Load())
	require.NoError(t, fixture.client.WriteMessage(websocket.TextMessage, []byte(`{"type":"response.create","model":"ws-billing","input":"hello","stream_id":"actual"}`)))
	var seenText bool
	for {
		event := readResponsesWSTestEvent(t, fixture.client)
		require.NotEqual(t, "error", event["type"], "%v", event)
		assert.Equal(t, "actual", event["stream_id"])
