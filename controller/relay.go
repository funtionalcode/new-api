...
func RelayResponsesWebsocket(c *gin.Context) {
	...
	selectChannel := func(ctx *gin.Context, modelName string) (int, *types.NewAPIError) {
		channel, selectErr := middleware.SelectChannelForWebsocketRequest(ctx, modelName)
		if selectErr != nil {
			return 0, selectErr
		}
		return channel.Id, nil
	}
	if newAPIError := relay.ResponsesWebsocketHelper(c, ws, selectChannel); newAPIError != nil {
...
func processChannelError(c *gin.Context, channelError types.ChannelError, err *types.NewAPIError, relayInfo *relaycommon.RelayInfo) {
	...
	if constant.ErrorLogEnabled && types.IsRecordErrorLog(err) {
		...
		if diagnostic, ok := common.GetContextKeyType[map[string]any](c, constant.ContextKeyUpstreamRequestDiagnostic); ok && len(diagnostic) > 0 {
			adminInfo["upstream_request"] = diagnostic
		}
