...
func AppendRelayLogAdminInfo(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, other *model.LogOther) {
	...
	if relayInfo != nil {
		if relayInfo.TypeSafeExchange != nil {
			other.SetAdmin("typesafe_exchange", relayInfo.TypeSafeExchange)
		}
		if len(relayInfo.TypeSafeResults) > 0 {
			other.SetAdmin("typesafe", relayInfo.TypeSafeResults)
			if summary := model.SanitizeTypeSafeResults(relayInfo.TypeSafeResults); len(summary) > 0 {
				other.SetPublic("typesafe", summary)
			}
		}
	...
	if diagnostic, ok := common.GetContextKeyType[map[string]any](ctx, constant.ContextKeyUpstreamRequestDiagnostic); ok && len(diagnostic) > 0 {
		other.SetAdmin("upstream_request", diagnostic)
	}
...
func GenerateTextOtherInfo(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, modelRatio, groupRatio, completionRatio float64,
	...
	if relayInfo.IsWebsocket {
		other.SetPublic("ws", true)
		other.SetPublic("transport", "websocket")
	}
	if lifecycle := common.GetContextKeyString(ctx, constant.ContextKeyCursorAgentLifecycle); lifecycle != "" {
		other.SetPublic("cursor_agent_lifecycle", lifecycle)
	}
