...
打开 TypeSafe 渠道的使用日志详情，展开 `TypeSafe 输入输出`，通过输入、输出页签查看并复制 JSON。
输入保存的是模型映射与参数覆盖后实际发送的请求体，包含 `model`、`state` 和 `questions`；输出保存的是上游成功返回的响应体。
辅助评估日志仍保留主请求 ID 和阶段，主模型日志仍记录评估结果摘要。

输入输出遵循现有管理员日志和渠道权限，仅在有权限的管理员日志详情中展示，不记录 HTTP 认证头。
每个方向最多保存 128 KiB，超过时显示截断提示；渠道 `max_chars` 对送往评估的内容仍然生效。
此功能只记录上线后的请求，历史日志未保存的输入输出无法补回。
