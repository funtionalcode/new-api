...
| GLM / DeepSeek 额度 | DeepSeek 今日消费改为展示今日消耗 Token，兼容 `today_token_usage` / `daily_token_usage` 字段，悬浮显示精确 token | `controller/deepseek_quota.go`、`model/deepseek_quota.go`、`web/default/src/features/quota-bindings` | 本次同步 |
