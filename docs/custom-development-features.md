...
| 登录认证 | Passkey 支持检测不再要求内置平台验证器，浏览器支持 WebAuthn 时允许外置安全密钥/跨平台通行密钥入口 | `web/default/src/lib/passkey.ts`、`web/default/src/lib/passkey.test.ts` | 本次同步 |
...
| Cliproxy 认证文件 | xAI 认证文件沿用认证文件表格展示，按 Claude/Codex 的进度条样式展示周限额、Api 使用和月度额度；悬浮展示各项重置时间、按需上限和账期结束时间，刷新时并行同步两类 billing 快照并保留部分成功数据 | `controller/cliproxy.go`、`model/cliproxy_auth_file.go`、`web/default/src/features/cliproxy-auth-files` | 本次同步 |
...
| 游乐场 | 图片/视频重试会保留视频模式并按当前模式纠正 xAI 图片/视频模型，Claude Fable 调用自动移除已废弃采样参数 | `relay/channel/claude`、`web/default/src/features/playground` | 本次同步 |
